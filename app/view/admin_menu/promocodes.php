...
if (isset($_POST['status']) && isset($_POST['id']) && isset($_POST['type'])) {
	GOIT_PRMCODE()->model->promocode->change_promocode_status($_POST['id'], $_POST['status'], $_POST['type']);
}
...
$promocodes = GOIT_PRMCODE()->model->promocode->get_promocodes($search_value);
$promo_counter = GOIT_PRMCODE()->model->promocode->get_promocodes_count($search_value); ?>

<section id="promocodes" class="goit">
	...
	<div class="goit-promocodes__table">
			...
			<?php if ($promocodes):
				foreach ($promocodes as $key => $item):
					...
					if (!empty($item->promocod_group) && $item->promocod_group == $data['group_name']
						...
						GOIT_PRMCODE()->view->load('admin_elements/promocode-group', $data);
						$table_items++;
					...
					if (count($promocodes) == 1 && !empty($item->promocod_group)) {
		...
		<div class="goit-promocodes__table-footer">
			<div class="counter-elements">
				<?php echo 'Кількість промокодів на сторінці: ' . $promo_counter; ?>
			</div>
		</div>
