...
class ZohoHelper
{
	...
	{
		$file_token = GOIT_PRMCODE_PATH . '/app/token.php';
	...
	public function getDataFromCrm($url, $param, $method)
	{
		...
		if ($result === false) {
			file_put_contents(GOIT_PRMCODE_PATH . '/app/logs/errorsLog.txt', wp_date('Y-m-d H:i:s') . "Error curl: " . curl_error($ch) . "\n", FILE_APPEND);
		}
	 ...
	 * Search record
	 */
	public function searchByCriteria($module_name, $criteria, $field_name = '')
	{
		...
		if (isset($result)) {
			...
			if (isset($result_array['status']) && $result_array['status'] == "error") {
				//log error to file
				file_put_contents(GOIT_PRMCODE_PATH . '/app/logs/errorsLog.txt', wp_date('Y-m-d H:i:s') . " Error $module_name: " . $result . "\n", FILE_APPEND);
...

	{
		...
		if (isset($result)) {
			...
			if ((isset($result_array['data'][0]['status']) && $result_array['data'][0]['status'] == "error") || (isset($result_array['status']) && $result_array['status'] == "error")) {
				//log error to file
				file_put_contents(GOIT_PRMCODE_PATH . '/app/logs/errorsLog.txt', wp_date('Y-m-d H:i:s') . "Error $module_name: " . $result . ' | ' . json_encode($module_data) . "\n", FILE_APPEND);
	 ...
	 * Create record
	 */
	public function createBulkRecords($module_name, $module_data, $field_name = '', $trigger = '') //$trigger = approval, workflow, blueprint
	 ...
	 * Get record
	 */
	public function getRelatedRecords($module_name, $related_module, $id, $field_name = '')
	{
		$url = "https://www.zohoapis.com/crm/v2/" . $module_name . "/" . $id . "/" . $related_module;
		//make request
		$result = $this->getDataFromCrm($url, array(), "GET");
